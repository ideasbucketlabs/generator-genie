    ...
    >
        <transition name="fade" @after-enter="show" @after-leave="onClose">
            <div class="absolute left-0 top-0 h-full w-full" v-if="displayOverlay" @click="emit('overlayClicked')">
                <div class="absolute left-0 top-0 h-full w-full bg-indigo-100 opacity-50 dark:bg-gray-700"></div>
            </div>
        </transition>
        <transition name="fade-bottom" @after-leave="displayOverlay = false">
            <div
                :class="width"
                class="absolute flex flex-col overflow-hidden rounded border border-indigo-300 bg-transparent shadow-lg dark:border-gray-800 dark:shadow-gray-900"
                v-if="displayContent"
            >
                <div
                    v-if="title"
                    class="flex items-center justify-between rounded-t border-b border-indigo-300 bg-indigo-500 px-2 py-2 dark:border-gray-800 dark:bg-gray-700"
                >
